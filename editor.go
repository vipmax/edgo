...
func (e *Editor) start() {
	...
	colors = highlighter.colorize(code)
...
func (e *Editor) handleEvents(s tcell.Screen) {
	...
	switch ev := ev.(type) { // Process event
		case *tcell.EventResize: COLUMNS, ROWS = s.Size(); ROWS -= 2; s.Sync(); s.Clear()
		case *tcell.EventKey:
			key := ev.Key()
			...
			if key == tcell.KeyCtrlS { e.writeFile(filename) }
			...
			if key == tcell.KeyLeft { e.onLeft() }
			if key == tcell.KeyRight { e.onRight() }
			if key == tcell.KeyTab { e.handleTabPress() }
...
func (e *Editor) onEnter(isSaveTabs bool) {
	...
	if isSaveTabs {
		...
		for i := 0; i < tabs; i++ { begining = append(begining, ' ') }
		c = tabs
	}
...
func (e *Editor) paste() {
	...
	for i, line := range lines {
		for _, char := range line { e.addChar(char) }
		if i < len(lines)-2 { e.onEnter(false) }
	}
}
...
func convertToString() string {
	...
	for _, row := range content { result.WriteString(string(row) + "\n") }
	return result.String()
}

func countTabsInRow(i int) int {
	if i < 0 || i >= len(content) { return 0 } // Invalid row index

	row := content[i]
	count := 0
	for _, char := range row { if char == ' ' { count++ } else { return count } }
	return count
}
